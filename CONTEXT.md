...
**Focus border**:
A persistent, click-through outline around the focused managed window, including tiled and floating windows, shown only while automatic tiling is active. Its only first-release customization is enabled state, color, and thickness; it is hidden in manual, suspended, and paused states.
